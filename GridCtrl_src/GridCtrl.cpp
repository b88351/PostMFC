...
BOOL  CGridCtrl::SetCellTextLen(int nRow, int nCol, short shLen)
{
    if (GetVirtualMode())
        return FALSE;
    ...
    pCell->SetStrLen(shLen);
    return  TRUE;
}

short CGridCtrl::GetCellTextLen(int nRow, int nCol)
{
    CGridCellBase* pCell = GetCell(nRow, nCol);
    ASSERT(pCell);
    if (!pCell)
    ...
    return pCell->GetStrLen();
}

BOOL CGridCtrl::SetColumnTextLen(int nCol, int strLen)
{
/*    ASSERT(nCol >= 0 && nCol < m_nCols && width >= 0);
    if (nCol < 0 || nCol >= m_nCols || width < 0)
        return FALSE;*/
    ...
    //m_arStrMaxLen[nCol] = strLen;
    //ResetScrollBars();

    CCellID idTopLeft = GetTopleftNonFixedCell();

    int i = 0;
    for(i=idTopLeft.row; i<=m_nRows - idTopLeft.row; i++)
    {
        SetCellTextLen(i, nCol, strLen);
    }
