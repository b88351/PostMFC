                           ...
                           UINT nFirstChar)
{
    ...
    //:: By Alex, Setting Limit text Length :://
    CGridCellBase *pCell = ((CGridCtrl*)pParent)->GetCell(m_nRow, m_nColumn);
    ASSERT(pCell);
    SetLimitText(pCell->GetStrLen());
    //:: By Alex                            :://
...
//::    By Alex - Remove the CEdit default right-click menu :://
void CInPlaceEdit::OnContextMenu(CWnd* pWnd, CPoint point) 
{
    ...
    CWnd* pOwner = GetOwner();
    if (pOwner)
        pOwner->SendMessage(WM_RBUTTONUP, 0, MAKELPARAM(point.x, point.y) );
}
//::    By Alex                                             :://